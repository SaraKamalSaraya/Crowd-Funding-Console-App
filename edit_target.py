...
def edit_target(email,title):
    user_email = email
    project_title = title
    print("Start Editing Target process \N{grinning face}")
    edit_target = Edit_Target(user_email,project_title)
            ...
            for project in projects:
                ...
                if self.email == project_info[0]:
                    if self.title == project_info[1]:
                        while True:
                            newTarget = input("Enter Your New Target : ")
                            if newTarget.isnumeric():
                                break
                            else:
                                print("Invalid input \N{unamused face}")
                        project_info[3] = newTarget
